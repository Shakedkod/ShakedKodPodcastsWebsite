...
import { MyPodcastId } from "@/constants";
import Link from "next/link";
import { getPodcast, urlFor } from "@/utils/sanity/client";
import { Podcast } from "@/utils/sanity/types";

const MyPodcasts = async () => {
    var podcasts: Podcast[] = [];

    for (let i = 0; i < MyPodcastId.length; i++) 
    {
        const podcast = await getPodcast(MyPodcastId[i]);
        ...
        <section id="podcasts" className="padding-container max-container flexCenter flex-col">
            ...
            <ul className="flexCenter flex-col md:flex-row gap-16 w-[95%]">
                {podcasts.map((podcast, index) => (
                    <li key={index} className="rounded-3xl bg-[#10586163] w-[300px] h-[400px] p-2 text-center">
